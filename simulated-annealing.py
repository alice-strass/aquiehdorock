...
def simulate_annealing(cidades, temperatura_maxima, temperatura_minima, numero_de_vizinhos, r_resfriamento, iteracoes):
    ...
    while temperatura >= temperatura_minima:
        ...
        while True:
            vizinhos = gerar_vizinhos(menor_caminho.copy(), numero_de_vizinhos)
            vizinho, distancia_do_vizinho = selecionar_menor_vizinho(vizinhos)

            if distancia_do_vizinho < menor_distancia:
                menor_caminho = vizinho.copy()
                menor_distancia = distancia_do_vizinho
            else:
                probabilidade = math.exp((menor_distancia - distancia_do_vizinho) / temperatura)
        ...
        temperatura = r_resfriamento * temperatura
...
caminho, distancia = simulate_annealing(
    cidades,
    temperatura_maxima=1000,
    temperatura_minima=25,
    r_resfriamento=0.95,
    iteracoes=50,
    numero_de_vizinhos=100
)
